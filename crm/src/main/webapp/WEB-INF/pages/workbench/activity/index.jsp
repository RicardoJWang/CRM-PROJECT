	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#saveCreateActivityBtn").click(function () {
				...
				const name = $.trim($("#create-marketActivityName").val());
				const startDate = $("#create-startDate").val();
				const endDate = $("#create-endDate").val();
				const cost = $.trim($("#create-cost").val());
			...
			$("#deleteActivityBtn").click(function(){
				...
				if(window.confirm("确定要删除掉这些市场活动吗?")){
					let ids = "";
			...
			$("#editActivityBtn").click(function(){
				const checkedId=$("#tBody input[type='checkbox']:checked");
				if (checkedId.size()==0){
					alert("请选择要修改的市场活动");
					return;
				}
			...
			$("#saveEditActivityBtn").click(function (){
				...
				const startDate=$("#edit-startTime").val();
				const endDate=$("#edit-endTime").val();
				const cost=$.trim($("#edit-cost").val());
				const description=$.trim($("#edit-describe").val());
			...
			$("#exportActivityXzBtn").click(function (){
				...
				if(window.confirm("确定要导出这些市场活动吗?")) {
					ids = "";
					$.each(checkedIds, function () {
						ids += "id=" + this.value + "&";
					});
			...
			$("#importActivityBtn").click(function (){
				const activityFileName = $("#activityFile").val();
				const suffix=activityFileName.substr(activityFileName.lastIndexOf(".")+1).toLowerCase();
				if (suffix!="xls"){
					alert("只能上传xls类型的文件");
					return;
				}
				const activityFile=$("#activityFile").get(0).files[0];
				...
				$.ajax({
					...
					success:function (data) {
						if (data.code=="1"){
							...
							alert(data.message);
							//模态窗口不关闭
							$("#importActivityModal").modal("show");//可以不写。
						}
					}
				});
			})
