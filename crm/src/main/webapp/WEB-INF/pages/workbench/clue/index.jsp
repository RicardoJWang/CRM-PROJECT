...
<meta charset="UTF-8">
	...
	$(function(){
		...
		$("#saveCreateClueBtn").click(function (){
			    ...
			    const company          =$.trim($("#create-company").val());
			    ...
			    const nextContactTime=$.trim($("#create-nextContactTime").val());
				...
				const b=/0\d{2,3}-\d{7,8}/;
				if (!b.test(phone)){
					alert("座机格式不合法!");
					return;
				}
			    const d=/^(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?$/;
			    if (!d.test(website)){
			    	alert("网站地址不合法!");
			    	return;
			    }
				const c=/^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/;
				if (!c.test(mphone)){
					alert("手机格式不合法!");
					return;
				}
	...
	function queryClueByConditionForPage(pageNo,pageSize) {
		...
		const name = $.trim($("#query-name").val());
		const owner = $.trim($("#query-owner").val());
		const phone  = $.trim($("#query-phone").val());
		const mphone = $.trim($("#query-mphone").val());
		const company = $.trim($("#query-company").val());
		const state  = $.trim($("#query-state").val());
		const source = $.trim($("#query-source").val());
		/*const pageNo = 1;
        const pageSize = 10;*/
